...
import numpy as np
import cv2
from charcterSegmation import char
...
def get_characters(raw_image,max_line_height,line,word):
	...
	rect.sort(key=lambda b: b[0],reverse=True)
	...
	# 	w = i[2]
	# 	h = i[3]
	# 	letter2 = mo_image[y:y+h,x:x+w]
	# 	cv2.imwrite('all_letters_in_img3/letter'+str(line)+'_'+str(word)+'_'+str(count2)+'.png', letter2)
	# 	count2 += 1
	...
	#Fix i and j
	rect = fix_i_j(rect, max_line_height, max_w,mo_image,line,word)
	...
	count = 0
	minrect = min(rect,key= lambda r:r[2])
	min_w = int(minrect[2])
	#raw_input('>')
	for i in rect:
		x = int(i[0])
		...
		cv2.imwrite('all_letters_in_img2/letter'+str(line)+'_'+str(word)+'_'+str(count)+'.png', letter)
		char('letter'+str(line)+'_'+str(word)+'_'+str(count), letter,10)
		...
		cv2.imwrite('all_letters_in_img/letter'+str(line)+'_'+str(word)+'_'+str(count)+'.png', letter)
	...
	cv2.imwrite("segmented/segmented"+str(line)+'_'+str(word)+".png", rect_segmented_image)
